"""OCR and parse scanned Snohomish County Affidavits of Publication.

The script:
- processes data/raw/affidavits;
- skips PDFs already represented by machine-readable stage records;
- caches OCR text locally;
- supports APN, numbered, and bare parcel anchors;
- preserves source pages and review metadata.
"""
...
SOURCE_DIR = PROJECT_ROOT / "data" / "raw" / "affidavits"
CACHE_DIR = PROJECT_ROOT / "data" / "private" / "ocr_cache" / "affidavits"
...
NUMBERED_ANCHOR_RE = re.compile(
    r"(?im)^[^A-Za-z0-9]{0,20}#?\s*(\d{1,4})[^A-Za-z0-9]+([0-9OIl|]{13,14})\b"
)
BARE_ANCHOR_RE = re.compile(
    r"(?im)^[^A-Za-z0-9]{0,20}([0-9OIl|]{13,14})\b"
)
...
def machine_readable_affidavit_filenames() -> set[str]:
    if not MACHINE_STAGE_PATH.exists():
        return set()

    df = pd.read_csv(MACHINE_STAGE_PATH)
    if "document_family" not in df.columns or "source_filename" not in df.columns:
        return set()

    return set(
        df.loc[
            df["document_family"].eq("affidavits"),
            "source_filename",
        ]
        .dropna()
        .astype(str)
...
def find_anchors(document_text: str) -> list[dict]:
    ...
    def add_candidate(match, raw_parcel, property_number, quality):
        normalized, repaired = normalize_parcel(raw_parcel)
        if not normalized:
            return
    ...
    for match in APN_ANCHOR_RE.finditer(document_text):
        add_candidate(
            ...
            match.group(1),
            None,
            "apn_context_validated",
        )

    for match in NUMBERED_ANCHOR_RE.finditer(document_text):
        add_candidate(
            match,
            match.group(2),
            int(match.group(1)),
    ...
    for match in BARE_ANCHOR_RE.finditer(document_text):
        add_candidate(
            match,
            match.group(1),
            None,
            "bare_context_validated",
        )
        ...
        records.append(
            OCRRecord(
                ...
                improvement_value=money(
                    first_line(text, r"IMPROVEMENT VALUE")
                ),
                ...
                extraction_method="tesseract_affidavit_cached_v1",
...
def main():
    ...
    skip_files = machine_readable_affidavit_filenames()
    pdf_paths = sorted(
        path
            ...
            file_stats.append(
                {
                    ...
                    "review_record_count": sum(
                        record.review_required_flag
                        for record in records
                    ),
                    "repaired_apn_count": sum(
                        record.parcel_repair_applied_flag
                        for record in records
                    ),
                    "parse_error": "",
                }
    ...
    if not records_df.empty:
        ...
        records_df.loc[mask, "review_reason"] = records_df.loc[
            ...
            lambda value: "; ".join(
                x
                for x in [value, "duplicate source key"]
                if x
            )
        )
    ...
    review_df = records_df[
        records_df["review_required_flag"]
    ].copy()

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    records_df.to_csv(
        OUTPUT_DIR / "ocr_affidavit_records.csv",
        index=False,
    )
    review_df.to_csv(
        OUTPUT_DIR / "ocr_affidavit_review_queue.csv",
        index=False,
    )
    files_df.to_csv(
        OUTPUT_DIR / "ocr_affidavit_file_summary.csv",
        index=False,
    )
    ...
    with pd.ExcelWriter(
        OUTPUT_DIR / "ocr_affidavit_records.xlsx",
        engine="openpyxl",
    ) as writer:
        excel_records.to_excel(
            writer,
            sheet_name="OCR Records",
            index=False,
        )
        excel_review.to_excel(
            writer,
    ...
    print(f"Affidavit PDFs processed: {len(pdf_paths):,}")
    print(f"Machine-readable affidavits skipped: {len(skip_files):,}")
    print(f"OCR records extracted: {len(records_df):,}")
    print(f"Review-queue records: {len(review_df):,}")
    print(
        "Repaired 13-digit APNs:",
        int(records_df["parcel_repair_applied_flag"].sum())
        if not records_df.empty
        else 0,
    )
    print(
        "Duplicate source keys:",
        int(records_df["duplicate_key_flag"].sum())
        if not records_df.empty
        else 0,
    )
    print("Output:", OUTPUT_DIR / "ocr_affidavit_records.xlsx")
